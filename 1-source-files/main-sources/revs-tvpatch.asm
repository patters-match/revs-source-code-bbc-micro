...
\ ------------------------------------------------------------------------------
\
\ This source file contains a small standalone binary that stands in for the
\ main game code on disc. The real game code (produced by revs-source.asm) is
\ catalogued as Revs3 instead of its usual Revs2, and this binary takes over
\ the Revs2 catalogue entry instead, so every existing loader's unmodified
\ *RUN/*LOAD Revs2 call - in Revs1.bin for the Acornsoft and 4 Tracks
\ releases, and the REVSMEN/REVSPLUS BASIC menu programs for the Superior and
\ Revs+ releases - runs this first without any of those loaders needing to
\ change at all. It then *LOADs Revs3, patches it to match the current *TV
\ setting, and jumps into it.
\
\ It's assembled to load and execute at &7000, deliberately outside the
\ &1200-&7000 range Revs3 occupies once loaded, so that neither the *LOAD
\ Revs3 call nor anything that runs after it is at risk of being overwritten
\ mid-load by the very file it's loading.
\
...
\ ******************************************************************************
 ...
 LOAD% = &7000          \ The load address of the code binary (deliberately
                        ...
                        \ it's loaded, unlike Revs3 which is assembled at a
...
\ REVS TVPATCH
\
\ Produces the binary file Revs2Patch.bin, which is catalogued as Revs2 in
\ place of the real game code.
\
\ ******************************************************************************
...
.Start
 ...
 LDX #LO(runRevs)       \ *LOAD Revs3 loads the file to its catalogued
 ...
 LDA &0291               \ Revs3 is assembled assuming *TV is interlaced
 BEQ Go                  \ (MOS's own default for this custom screen mode,
                        ...
                        \ routine). If *TV is interlaced, Revs3's own
...
.Go
 ...
 JMP &1200               \ Jump to Revs3's own entry point (its catalogued
...
\ ******************************************************************************
 ...
 SAVE "3-assembled-output/Revs2Patch.bin", LOAD%, P%
